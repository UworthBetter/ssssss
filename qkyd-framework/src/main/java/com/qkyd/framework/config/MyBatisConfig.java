...
public class MyBatisConfig {
    ...
    public SqlSessionFactory sqlSessionFactory(DataSource dataSource) throws Exception {
        ...
        if (StringUtils.isEmpty(typeAliasesPackage)) {
            typeAliasesPackage = env.getProperty("mybatis.type-aliases-package"); // 尝试读取 kebab-case 格式
        }
        if (StringUtils.isEmpty(typeAliasesPackage)) {
            // 如果都读取不到，给一个默认值，避免 NPE 导致服务无法启动
            typeAliasesPackage = "com.qkyd.**.domain";
        }
        ...
        if (StringUtils.isEmpty(configLocation)) {
            configLocation = env.getProperty("mybatis.config-location");
        }
        if (StringUtils.isEmpty(configLocation)) {
