...
public class UeitExceptionServiceImpl implements IUeitExceptionService
{
    ...
    {
        normalizeCreateTimeRange(ueitException);
     ...
     * 根据用户id查询异常数据列表
     *
     * @param ueitException 查询条件
     * @return 异常数据列表
     */
    @Override
    public List<UeitException> selectUeitExceptionListByUserId(UeitException ueitException)
    {
        normalizeCreateTimeRange(ueitException);
        return ueitExceptionMapper.selectUeitExceptionListByUserId(ueitException);
    }

    private void normalizeCreateTimeRange(UeitException ueitException)
    {
        try
        {
