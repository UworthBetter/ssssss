...
public class UeitLocationController extends BaseController
{
    ...
    public void export(HttpServletResponse response, UeitLocation ueitLocation)
    {
        ...
        util.exportExcel(response, list, "定位数据数据");
    }
    ...
    @Log(title = "定位数据", businessType = BusinessType.INSERT)
    @PostMapping
    ...
    @Log(title = "定位数据", businessType = BusinessType.UPDATE)
    @PutMapping
    ...
    @Log(title = "定位数据", businessType = BusinessType.DELETE)
	@DeleteMapping("/{ids}")
